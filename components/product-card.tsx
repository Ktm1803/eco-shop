...
export function ProductCard({ product }: ProductCardProps) {
  ...
  // Get product name - either translated or original
  const productName = translateProductContent
  ...
  const handleAddToCart = useCallback(
    (e: MouseEvent<HTMLButtonElement>) => {
      ...
      // Đảm bảo quantity luôn là 1 khi thêm từ product card
      addItem({
        id: product.id,
        name: product.name,
      ...
      console.log(`Added product ${product.id} to cart with quantity 1`)

      // Hiển thị toast thông báo
      toast({
        title: t("common.added_to_cart"),
        description: `${productName} ${t("common.added_to_cart").toLowerCase()}`,
      })
    },
    [addItem, discountedPrice, product.id, product.image, product.name, productName, user, router, toast, t],
  )
      ...
      <Link href={`/products/${product.id}`} className="block group">
        <Card className="overflow-hidden h-full transition-all duration-200 hover:shadow-lg border-transparent hover:border-primary/20">
          ...
          <CardContent className="p-4">
            <h3 className="font-medium line-clamp-1 group-hover:text-primary transition-colors">{productName}</h3>
